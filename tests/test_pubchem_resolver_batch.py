...
def test_name_to_smiles_pubchem_batch_basic_mapping(monkeypatch):
    """name_to_smiles_pubchem_batch should map each name to the SMILES returned by PubChem."""
    ...
    monkeypatch.setattr(
        "cholla_chem.resolvers.pubchem_resolver.pubchem_resolver_batch.filter_latin1_compatible",
    ...
    monkeypatch.setattr(
        "cholla_chem.resolvers.pubchem_resolver.pubchem_resolver_batch.get_compounds",
        fake_get_compounds,
        raising=True,
    )

    compound_names = ["ethanol", "water", "acetone"]
    result = pcr.name_to_smiles_pubchem_batch(compound_names)
...
def test_name_to_smiles_pubchem_batch_handles_none_results(monkeypatch):
...
def test_name_to_smiles_pubchem_batch_logs_length_mismatch(monkeypatch):
    ...
    monkeypatch.setattr(
        "cholla_chem.resolvers.pubchem_resolver.pubchem_resolver_batch.logger",
        FakeLogger,
        raising=True,
    )
